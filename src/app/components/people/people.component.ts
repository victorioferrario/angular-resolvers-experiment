...
export class PeopleComponent implements AfterViewInit {
  ...
  showAvatars() {
    ...
    const keyframes = (index: number): any[] => {
      return [
        /* It starts in a random position: */
        {
          transform: `translate(${random(10, 350)}px, ${random(
            -10,
            -550
          )}px) scale(0)`
        },
    ...
    //   .pipe(take(this.people.length))
    //   .subscribe(index => {
    //     document
    //       .getElementById(`person_${index + 1}`) // Web Animations API
